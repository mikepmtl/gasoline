...
/**
 * GasAuth ORM driven acl driver
 *
 * @package     Gasoline
 * @subpackage  Auth
 */

class Auth_Acl_Gasacl extends \Auth_Acl_Driver
{
    /*
    ...
    public static function _init()
    {
        ...
        try
        {
            static::$_valid_roles = \Cache::get(\Config::get('gasauth.cache_prefix', 'auth').'.roles');
        }
        catch (\CacheNotFoundException $e)
        {
            static::$_valid_roles = \Model\Auth_Role::find('all');
            \Cache::set(\Config::get('gasauth.cache_prefix', 'auth').'.roles', static::$_valid_roles);
        }
    }
    ...
    public function has_access($condition, Array $entity)
    {
        ...
        $cache_key = \Config::get('gasauth.cache_prefix', 'auth').'.permissions.user_'.($user ? $user->id : 0);
        try
        {
...
/* End of file gasacl.php */
/* Location: ./fuel/gasoline/classes/auth/login/gasacl.php */
