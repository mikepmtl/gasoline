...
/**
 * GasAuth ORM driven login driver
 *
 * @package     Gasoline
 * @subpackage  Auth
 */

class Auth_Login_Gasauth extends \Auth\Auth_Login_Driver
{
    /*
    ...
    public static function _init()
    {
        ...
        \Config::load('gasauth', true, true, true);

        // deal with invalid column selections
        if ($columns = \Config::get('gasauth.table_columns', array()) == array('*') or ! is_array($columns))
        {
            \Config::set('gasauth.table_columns', array());
        }

        // setup the remember-me session object if needed
        if (\Config::get('gasauth.remember_me.enabled', false))
        {
            static::$remember_me = \Session::forge(array(
                'driver' => 'cookie',
                'cookie' => array(
                    'cookie_name' => \Config::get('gasauth.remember_me.cookie_name', 'rmcookie'),
                ),
                'encrypt_cookie' => true,
                'expire_on_close' => false,
                'expiration_time' => \Config::get('gasauth.remember_me.expiration', 86400 * 31),
            ));
        }
    }
    ...
    /**
     * @var  array  Gasauth class config
     */
    protected $config = array(
        'drivers' => array('group' => array('Gasgroup')),
    ...
    public function validate_user($username_or_email = '', $password = '')
    {
        ...
        $username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('gasauth.username_post_key', 'username')));
        $password = trim($password) ?: trim(\Input::post(\Config::get('gasauth.password_post_key', 'password')));
        ...
        $user = \Model\Auth_User::query()
            ->select(\Config::get('gasauth.table_columns', array()))
            ->related('metadata')
            ->where_open()
        ...
        if ( $user )
        {
            return ( $this->match_password($user->password, $password) ? $user : false );
        }
    ...
    public function logout()
    {
        ...
        if (\Config::get('gasauth.guest_login', true))
        {
            $this->user = \Model\Auth_User::query()
                ->where('id', '=', 0)
                ->get_one();
    ...
    public function update_user($values, $username = null)
    {
        ...
        if (array_key_exists('password', $values))
        {
            if ( empty($values['old_password'])
                OR ! $ths->match_password($current_values->password, $values['old_password']) )
            {
    ...
    public function create_login_hash()
    {
        ...
        $this->user->login_hash = sha1(\Config::get('gasauth.login_hash_salt').$this->user->username.$this->user->last_login);
    ...
    public function get_groups()
    {
        ...
        return array(array('Gasgroup', $this->user->group));
    }
    ...
    {
        return \Config::get('gasauth.guest_login', true);
    }
    
    /**
     * Hash the password using crypt and a random salt
     * 
     * @param   string  $password   The passwort to hash
     * @param   string  $salt       A random and crypt-valid salt to use
     * 
    ...
    protected function perform_check()
    {
        ...
        if ( ! empty($username) and ! empty($login_hash))
        {
            ...
            if ($this->user and (\Config::get('gasauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
            {
                return true;
            }
        }
    ...
    public function match_password($original, $to_match)
    {
        // Extract the salt from the original password
        $salt = substr($original, 0, 29);
        
        // Create a new hash with the password to-match
        $new_hash = $this->hash_password($to_match, $salt);
        
        // And see if the given passwords match
        return ( $original === $new_hash );
    }
}

/* End of file gasauth.php */
/* Location: ./fuel/gasoline/classes/auth/login/gasauth.php */
